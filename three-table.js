...
export class HighStakesTable {
  constructor(canvas) {
    ...
    this.cameraHome = new THREE.Vector3(0, 8.2, 12.8);
    this.cameraAnchor = this.cameraHome.clone();
    this.focusAnchor = new THREE.Vector3(0, 0.15, 0.2);
  ...
  build() {
    this.ambient = new THREE.HemisphereLight(0xb7d9c9, 0x180b05, 2.25);
    this.scene.add(this.ambient);

    this.key = new THREE.SpotLight(0xffd77e, 82, 42, Math.PI / 5.2, 0.48, 1.15);
    ...
    this.fill = new THREE.PointLight(0x66bd91, 19, 22);
    this.fill.position.set(-4.8, 3.2, 1.8);
    this.scene.add(this.fill);

    this.red = new THREE.PointLight(0xb92d2d, 9, 18);
  ...
  buildTable() {
    const wood = this.material(0x4b210c, 0.46, 0.08);
    const felt = this.material(0x087044, 0.9, 0.02);
    const brass = this.material(0xd2a43e, 0.25, 0.8);

    const base = new THREE.Mesh(
      new THREE.CylinderGeometry(5.35, 5.5, 0.5, 72),
      wood
    );
    ...
    const top = new THREE.Mesh(
      new THREE.CylinderGeometry(4.87, 4.87, 0.2, 72),
      felt
    );
    ...
    const rail = new THREE.Mesh(
      new THREE.TorusGeometry(4.04, 0.31, 20, 100),
      wood
    );
    ...
    const trim = new THREE.Mesh(
      new THREE.TorusGeometry(3.72, 0.06, 12, 100),
      brass
    );
    ...
    const innerLine = new THREE.Mesh(
      new THREE.TorusGeometry(2.95, 0.022, 8, 90),
    ...
    innerLine.scale.y = 0.7;
    innerLine.position.y = 0.075;
  ...
  buildZones() {
    ...
    const make = (key, label, subtitle, x, color) => {
      ...
      const zone = new THREE.Mesh(
        new THREE.PlaneGeometry(2.7, 1.33),
        material
      );
      zone.rotation.x = -Math.PI / 2;
      zone.position.set(x, 0.105, 1.18);
      this.root.add(zone);
      ...
      frame.scale.y = 0.56;
      frame.position.set(x, 0.12, 1.18);
      this.root.add(frame);
  ...
  buildChips() {
    ...
    this.playerGroup.position.set(0, 0.06, 3.05);
  ...
  async dealFact(fact) {
    ...
    this.cardOuter.rotation.set(0, -0.05, -0.07);
  ...
  async chooseAnswer(choice) {
    ...
    await this.tween(540, (progress) => {
      ...
      this.potGroup.position.z = startZ + (1.08 - startZ) * eased;
  ...
  async resetRound() {
    ...
    this.key.intensity = 82;
    this.fill.intensity = 19;
    this.red.intensity = 9;
